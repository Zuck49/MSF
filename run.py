...
_____ListID_______ = [
    "100068397981838","100068397981838","100068397981838","100068397981838","100068397981838]
_____Post_______= [
    "254018450221419","254018450221419"]
_____INFO______ = 'Bang lu sange? ya bang sini saya kocok in, Bang lu mau gua sepongin gk, Sampe ngecrot  ya bang wkkwkw & Santai aja kali bang'
...
{H} ____    ____   ______   ________  • By Niki  •
|_   \  /   _|.' ____ \ |_   __  | • Facebook•Niki mark •
{K}  |   \/   |  | (___ \_|  | |_ \_|• INSTAGRAM.COM/goleradya •
  | |\  /| |   _.____`.   |  _|    
{B} _| |_\/_| |_ | \____) | _| |_     
{U}|_____||_____| \______.'|_____|    
"""%(N))
...
def _____bot________():
	...
	______kom______ = ('Halo Bambang Niki Yang Super ganteng') 
...
def __cekakun__():
    ...
    if anjg == '':
        ...
        totalcp = open('ok.txt').read().splitlines()
        ______SAYANGKAMU______(' cat ok.txt')
        ______DickyGans______('\n [\xe2\x80\xa2] kembali ')
        ___SayaRecodeSampah___()
    else:
...
def ____MANUAL_____():
	...
	if pwek=="":
		...
		______LuWibu______("\n\n [#] crack selesai...")
def apii(uid, pwx):
	global _______ok_______, _______ok_______, _______loop______
	...
	for wk in list('\|-/'):
		sys.stdout.write("\r %s[%s%s%s] [ %sCrack %s] %s/%s OK/%s - OK/%s "%(rgb,K,wk,rgb,rgb,rgb,_______loop______, len(id), len(_______ok_______), len(_______ok_______)))
		sys.stdout.flush()
